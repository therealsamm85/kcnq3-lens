...
Upload two EEG recordings and see what changed between them. This is the single most valuable feature for any family tracking medication response — exactly the workflow we used to compare pre-Sultiam vs post-Sultiam in Liyana's case.
...
The 30-min-bin chart we built to find Liyana's first-NREM-cycle activation peak. Useful for: identifying when in the night spikes cluster, comparing across nights, showing the doctor the "shape" of the night.
...
- **Diagnostic claims.** This will never be a "Liyana has CSWS / R230H disorder" output. It surfaces patterns; doctors interpret.
