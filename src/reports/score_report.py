...
def _gnum(findings: dict, *path):
    """Pull a numeric finding, returning None for missing OR non-finite values
    so a NaN/inf never renders as a real clinical number."""
    v = _g(findings, *path)
    if v is None:
        return None
    try:
...
def build_score_report(findings: dict) -> ScoreReport:
    ...
    pdr = _gnum(findings, "background", "posterior_dominant_rhythm_hz")
    if pdr is not None:
        line = f"Posterior dominant rhythm: {pdr:g} Hz"
        corr = _gnum(findings, "background", "pdr_aperiodic_corrected_hz")
        if corr is not None:
    ...
    dar = _gnum(findings, "background", "delta_alpha_ratio")
    if dar is not None:
    ...
    # ── Sleep ──────────────────────────────────────────────────────────────
    sl = sections["Sleep"]
    eff = _gnum(findings, "sleep_stages", "sleep_efficiency_pct")
    spin = _gnum(findings, "spindles", "density_per_minute")
    if eff is not None:
    ...
    spike_rate = _gnum(findings, "morphology", "events_per_minute")
    ...
    sharp = _gnum(findings, "sharp_spikes", "events_per_minute")
    if sharp is not None:
        ie.append(f"Broadband sharp-spike rate: {sharp:g}/min")
    swi = _gnum(findings, "swi", "swi_n3_only_pct")
    if swi is not None:
    ...
    n_cand = _gnum(findings, "ictal", "n_candidates")
    if n_cand is not None:
        if n_cand > 0:
            ic.append(f"{int(n_cand)} candidate electrographic seizure(s) flagged "
                      "for human review (screening only — not confirmed).")
            impression.append(f"{int(n_cand)} ictal candidate(s) to review")
        else:
    ...
    hfo = _gnum(findings, "hfo_ripples", "rate_per_min")
    if hfo is not None:
        oq.append(f"HFO/ripple rate: {hfo:g}/min")
    spk_hfo = _gnum(findings, "hfo_classify", "n_spike_coupled")
    if spk_hfo is not None:
        oq.append(f"Spike-coupled HFOs (spkHFO): {int(spk_hfo)}")
    wpli = _g(findings, "connectivity", "mean_wpli_by_band")
    if isinstance(wpli, dict) and wpli:
        alpha = _gnum(wpli, "alpha")
        if alpha is not None:
            oq.append(f"Mean alpha wPLI: {alpha:g}")
    ent = _g(findings, "entropy", "metrics")
    if isinstance(ent, dict):
        samp = _gnum(ent, "sample_entropy")
        if samp is not None:
            oq.append(f"Sample entropy: {samp:g}")
    if not oq:
