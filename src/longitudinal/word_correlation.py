...
* only ever calls a relationship "statistically conclusive" at or above that
  point count with an EXACT permutation p < 0.05 (never scipy's asymptotic
  approximation, which is anti-conservative at small n and would over-call it),
...
def compute_word_correlation(
    ...
    for metric in metrics:
        ...
        for ds, v in zip(dates, vals):
            d = parse_date(ds)
            fv = float(v)
            # Drop non-finite findings: an inf survives the rank-based Spearman
            # (it just becomes the extreme rank) and would yield an ordinary-
            # looking correlation built on garbage.
            if d is not None and np.isfinite(fv):
                metric_series.append((d, fv))
...
_EXACT_PERM_MAX_N = 8        # n! ≤ 40320 — full enumeration is sub-second
_MC_PERM_RESAMPLES = 20000


def _spearman_permutation_p(mvals: np.ndarray, wvals: np.ndarray) -> float | None:
    """Exact (n ≤ 8) / Monte-Carlo two-sided permutation p for Spearman rho.

    Spearman rho is Pearson on ranks, so the permutation null is generated by
    permuting one rank vector against the other and counting how often the
    permuted |rho| reaches the observed |rho|. Exact by enumerating all n!
    pairings for small n; sampled above the enumeration cap (with an add-one
    correction and a fixed seed so the result is reproducible). Returns None if
    either rank vector has no spread. Tie-correct because it ranks first.
    """
    n = len(mvals)
    rm = rankdata(mvals).astype(float)
    rw = rankdata(wvals).astype(float)
    rm -= rm.mean()
    rw -= rw.mean()
    denom = float(np.sqrt((rm ** 2).sum() * (rw ** 2).sum()))
    if denom == 0.0:
        return None
    obs = abs(float((rm * rw).sum()) / denom)
    eps = 1e-12
    if n <= _EXACT_PERM_MAX_N:
        count = total = 0
        for perm in permutations(range(n)):
            stat = abs(float((rm * rw[list(perm)]).sum()) / denom)
            total += 1
            if stat >= obs - eps:
                count += 1
        return count / total
    ...
    idx = np.arange(n)
    count = 0
    for _ in range(_MC_PERM_RESAMPLES):
        rng.shuffle(idx)
        stat = abs(float((rm * rw[idx]).sum()) / denom)
        if stat >= obs - eps:
            count += 1
    return (count + 1) / (_MC_PERM_RESAMPLES + 1)
...
def _assess_metric(metric: str, pairs: list[PairedPoint]) -> MetricWordCorr:
    ...
    rho_raw = spearmanr(mvals, wvals).statistic
    ...
    # p-value only when n is large enough for it to mean anything. Use the EXACT
    # two-sided permutation p, NOT scipy's default asymptotic t-approximation,
    # which is anti-conservative at small n: at n=8 the asymptotic p crosses 0.05
    # near |rho|=0.71 while the exact permutation p does not until |rho|≈0.74 — so
    # the asymptotic p would fabricate significance on a real child's case.
    show_p = n >= MIN_PAIRS_FOR_SIGNIFICANCE
    p_raw = _spearman_permutation_p(mvals, wvals) if show_p else None
    p_value = round(float(p_raw), 4) if p_raw is not None else None
